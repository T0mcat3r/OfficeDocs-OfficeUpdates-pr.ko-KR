...
*2018년 1월부터 제공된 반기 채널 릴리스입니다. 이 릴리스는 2019년 3월까지 계속 지원되며 보안 업데이트를 받게 됩니다다. 그러나 이제 새로운 반기 채널 릴리스 버전 1803(빌드 16.0.9126.2282)을 사용할 수 있습니다. 여기에는 새로운 기능, 보안 업데이트 및 비보안 업데이트가 포함되어 있습니다.*
...
-   [Excel: 보안 업데이트](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-1029)
...
-   **3D 모델:** 3D를 사용하여 프레젠테이션의 시각적 효과 및 창의적 효과를 향상시킬 수 있습니다. 슬라이드 사이에 시시네마틱 애니메이션을 만드는 모핑 같은 전환을 통해 프레젠테이션에서 3D 모델을 구현할 수 있습니다. [자세한 정보](https://support.office.com/article/ec5feb79-b0af-47f6-a885-151fcc88ac0a)
-   **새 잉크 효과:** 금속 펜과 무지개, 은하수, 용암, 바다, 금색, 은색 등의 잉크 효과를 사용하여 아이디어를 세련되게 표현할 수 있습니다.
-   **파일 공유 UI:** 비즈니스용 OneDrive 또는 SharePoint 파일의 경우, 리본의 오른쪽 상단 모서리에 있는 공유 단추를 클릭하거나 파일 \> 공유로 이동하면 간단하게 개선된 공유 대화 상자가 나타납니다. UI는 사용자가 새 파일 또는 로컬에 저장된 파일을 쉽게 OneDrive에 업로드하여 공동 작업을 시작할 수 있게 합니다.
-   **위험한 확장명 차단:** 위험을 초래할 것으로 간주되며, OLE 패키지 개체로 포함되어 있는 확장명은 기본적으로 활성화되지 않도록 차단됩니다. 예로 .exe, .vbs 및 .js가 있습니다. [자세한 정보](https://support.office.com/article/packager-activation-in-office-365-desktop-applications-52808039-4a7c-4550-be3a-869dd338d834)
...
-   **작업 보드 보기:** 작업 보드 보기에서 카드의 작업을 정렬합니다. Agile 프로젝트에서와 같이 보드의 열 사이에서 카드를 재정렬하고 이동합니다.
...
-   **Planner에서 작업 관리:** Planner에 프로젝트트 작업을 연결하고 계획을 세울 수 있습니다. 작업을 하위 작업으로 나누고, 팀을 추가하고, 작업을 할당하고 작업 보드에서 작업을 관리합니다.
...
-   타임라인 보기 상태에서 타임라인에 기존 작업을 추가할 때 첫 번째 요약 작업의 작업만 대화 상자에 표시되는 문제를 해결합니다.
-   세션 모음에서 둘 이상의 기준선을 설정하면 MOD\_DATE 값이 동일하게 설정되는 문제를 수정합니다.
...
-   **파일 공유 UI:** 비즈니스용 OneDrive 또는 SharePoint 파일의 경우, 리본의 오른쪽 상단 모서리에 있는 공유 단추를 클릭하거나 파일 \> 공유로 이동하면 간단하게 개선된 공유 대화 상자가 나타납니다. UI는 사용자가 새 파일 또는 로컬에 저장된 파일을 쉽게 OneDrive에 업로드하여 공동 작업을 시작할 수 있게 합니다.
-   **위험한 확장명 차단:** 위험을 초래할 것으로 간주되며, OLE 패키지 개체로 포함되어 있는 확장명은 기본적으로 활성화되지 않도록 차단됩니다. 예로 .exe, .vbs 및 .js가 있습니다. [자세한 정보](https://support.office.com/article/packager-activation-in-office-365-desktop-applications-52808039-4a7c-4550-be3a-869dd338d834)
-   **학습 도구를 사용한 편집: **이제 Word의 웹 레이아웃에서 학습 도구를 사용할 수 있습니다. 편집하는 동안 텍스트 간격을 조정하고 음절을 표시합니다. 모든 뷰에서 문서를 소리내어 읽을 때 각 단어가 강조 표시되는 것을 확인합니다.[ 자세한 정보](https://support.office.com/article/a857949f-c91e-4c97-977c-a4efcaf9b3c1)
...
-   System Center 구성 관리자를 사용하여 Office 응용 프로그램을 실행하는 클라이언트에 업데이트를 배포할 때 Office 응용 프로그램이 실행되는 동안 장치를 다시 시작한 후에 업데이트가 적용되지 않는 문제를 해결합니다.
...
-   .box 파일이 동일한 컴퓨터의 모든 Office 앱 버전에서 공유되므로 .box 파일 형식이 변경될 경우 같은 컴퓨터에 설치된 이전 Office 버전의 기능에 영향을 미치는 문제를 해결합니다.
...
### <a name="outlook-feature-updates"></a>Outlook: 기능 업데이트
-   **접근성 개선:** 화면 판독기를 사용하는 경우 전자 메일에서 텍스트, 표, 목록 및 이미지를 더욱 쉽게 읽고 편집할 수 있습니다.
...
-   **Office 365의 그룹에 대한 게스트 액세스:** 그룹 대화, 파일, 일정 초대 및 그룹 전자 필기장에 대한 액세스 권한을 부여하여 조직 외부의 사용자와 공동으로 작업합니다. [추가 정보](https://support.office.com/article/bfc7a840-868f-4fd6-a390-f347bf51aff6)
...
-   저장에 실패하면 파일이 손상되고 프로젝트가 열려 있을 때 중지되는 문제를 수정합니다.
...
-   로컬 리소스를 대체하고 보호된 작업을 사용할 때 실제 작업이 Enterprise 자원으로 잘못 전송되는 문제를 수정합니다.
...
-   최상위 수준의 작업 들여쓰기가 프로젝트에서 SharePoint 작업 목록으로 올바르게 동기화되지 않는 문제를 수정합니다.
...
-   IM 대화 탭을 클릭하면 영구 채팅방의 읽지 않은 메시지가 읽음으로 표시되는 문제를 수정합니다.
...
-   비즈니스용 Skype와 비즈니스용 Skype Basic 간을 이동하는 사용자의 대리자가 삭제되는 문제를 수정합니다.
...
-   그리기 캔버스 내의 도형을 회전할 수 없는 문제를 수정합니다.
-   한국어를 입력할 때 자음과 모음이 정확하지 않게 구분되는 문제를 수정합니다.
...
-   일부 프로그램 파일이 서명되는 방식으로 인해 바이러스 백신 프로그램이 이러한 파일에 플래그를 표시하는 문제와 WIP(Windows Information Protection)의 데이터 보호 및 액세스에 발생하는 문제를 수정합니다.
